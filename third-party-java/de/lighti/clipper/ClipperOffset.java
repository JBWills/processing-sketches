...
public class ClipperOffset {
  ...
  public void addPath(Path path, JoinType joinType, EndType endType) {
    ...
    for (int i = 1; i <= highI; i++) {
      if (newNode.getPolygon().get(j) != path.get(i)) {
        ...
        if (path.get(i).getY() > newNode.getPolygon().get(k).getY() || path.get(
          i).getY() == newNode.getPolygon().get(k).getY()
    ...
    if (lowest.getX() < 0) {
      ...
      final LongPoint ip = polyNodes.getChilds().get((int) lowest.getX()).getPolygon().get(
        (int) lowest.getY());
      if (newNode.getPolygon().get(k).getY() > ip.getY() || newNode.getPolygon().get(
        k).getY() == ip.getY()
  ...
  private void doMiter(int j, int k, double r) {
    ...
    destPoly.add(new LongPoint(
      (int) Math.round(srcPoly.get(j).getX() + (normals.get(k).getX() + normals.get(j).getX()) * q),
      (int) Math
    ...
    for (int i = 0; i < polyNodes.getChildCount(); i++) {
      ...
      if (len == 1) {
        if (node.getJoinType() == JoinType.ROUND) {
          ...
          for (int j = 1; j <= steps; j++) {
            destPoly.add(new LongPoint(
              (int) Math.round(srcPoly.get(0).getX() + X * delta),
              (int) Math.round(srcPoly.get(0).getY() + Y
                * delta)
            ));
            final double X2 = X;
            X = X * cos - sin * Y;
      ...
      if (node.getEndType() == EndType.CLOSED_POLYGON) {
        ...
        if (node.getEndType() == EndType.OPEN_BUTT) {
          final int j = len - 1;
          pt1 = new LongPoint(
            (int) Math.round(srcPoly.get(j).getX() + normals.get(j).getX() * delta),
            (int) Math.round(srcPoly.get(j)
        ...
        if (node.getEndType() == EndType.OPEN_BUTT) {
          pt1 = new LongPoint(
            (int) Math.round(srcPoly.get(0).getX() - normals.get(0).getX() * delta),
            (int) Math.round(srcPoly.get(0)
  ...
  private void doRound(int j, int k) {
    ...
    for (int i = 0; i < steps; ++i) {
      destPoly.add(new LongPoint(
        (int) Math.round(srcPoly.get(j).getX() + X * delta),
        (int) Math.round(srcPoly.get(j).getY() + Y * delta)
      ));
      X2 = X;
      X = X * cos - sin * Y;
      Y = X2 * sin + Y * cos;
    ...
    destPoly.add(
      new LongPoint(
        (int) Math.round(srcPoly.get(j).getX() + normals.get(j).getX() * delta),
        (int) Math.round(srcPoly.get(j).getY()
  ...
  private void fixOrientations() {
    ...
    if (lowest.getX() >= 0 && !polyNodes.childs.get(
      (int) lowest.getX()).getPolygon().orientation()) {
      for (int i = 0; i < polyNodes.getChildCount(); i++) {
  ...
  private void offsetPoint(int j, int[] kV, JoinType jointype) {
    ...
    if (Math.abs(inA * delta) < 1.0) {
      ...
      {
        destPoly.add(
          new LongPoint((int) Math.round(sjx + nkx * delta), (int) Math.round(sjy + nky * delta),
            0
          ));
        return;
      }
    ...
    if (inA * delta < 0) {
      ...
      destPoly.add(
        new LongPoint((int) Math.round(sjx + njx * delta), (int) Math.round(sjy + njy * delta)));
    } else {
      switch (jointype) {
        case MITER: {
