...
public class DefaultClipper extends ClipperBase {
  ...
  private static boolean joinPoints(Join j, OutRec outRec1, OutRec outRec2) {
    ...
    if (isHorizontal && j.getOffPt().equals(j.outPt1.getPt()) && j.getOffPt().equals(
      ...
      if (!getOverlap(
        op1.getPt().getX(), op1b.getPt().getX(), op2.getPt().getX(), op2b.getPt().getX(), LeftV,
        RightV
      )) {
        return false;
      }
      ...
      final boolean Reverse1 = op1b.getPt().getY() > op1.getPt().getY() || !Point.slopesEqual(
        op1.getPt(), op1b.getPt(), j.getOffPt());
      if (Reverse1) {
        op1b = op1.prev;
        while (op1b.getPt().equals(op1.getPt()) && op1b != op1) {
          op1b = op1b.prev;
        }
        if (op1b.getPt().getY() > op1.getPt().getY() || !Point.slopesEqual(
          op1.getPt(), op1b.getPt(), j.getOffPt())) {
          return false;
        }
      }
      ...
      final boolean Reverse2 = op2b.getPt().getY() > op2.getPt().getY() || !Point.slopesEqual(
        op2.getPt(), op2b.getPt(), j.getOffPt());
      if (Reverse2) {
        op2b = op2.prev;
        while (op2b.getPt().equals(op2.getPt()) && op2b != op2) {
          op2b = op2b.prev;
        }
        if (op2b.getPt().getY() > op2.getPt().getY() || !Point.slopesEqual(
          op2.getPt(), op2b.getPt(), j.getOffPt())) {
          return false;
        }
      }
  ...
  private Path.OutPt addLocalMinPoly(Edge e1, Edge e2, LongPoint pt) {
    ...
    if (prevE != null && prevE.outIdx >= 0 && Edge.topX(prevE, pt.getY()) == Edge.topX(
      e, pt.getY()) && Edge.slopesEqual(e, prevE)
  ...
  private void fixupOutPolygon(OutRec outRec) {
    ...
    for (; ; ) {
      ...
      if (pp.getPt().equals(pp.next.getPt()) || pp.getPt().equals(pp.prev.getPt())
        ...
        && (!preserveCollinear || !Point.isPt2BetweenPt1AndPt3(
        pp.prev.getPt(), pp.getPt(), pp.next.getPt()))) {
        lastOK = null;
    ...
    while (currentLM != null && currentLM.y == botY) {
      ...
      if (Op1 != null && rb.isHorizontal() && ghostJoins.size() > 0 && rb.windDelta != 0) {
        for (int i = 0; i < ghostJoins.size(); i++) {
          ...
          if (doHorzSegmentsOverlap(
            j.outPt1.getPt().getX(), j.getOffPt().getX(), rb.getBot().getX(), rb.getTop().getX())) {
      ...
      if (lb.nextInAEL != rb) {
        ...
        if (rb.outIdx >= 0 && rb.prevInAEL.outIdx >= 0 && Edge.slopesEqual(
          rb.prevInAEL, rb) && rb.windDelta != 0 && rb.prevInAEL.windDelta != 0) {
  ...
  private void intersectEdges(Edge e1, Edge e2, LongPoint pt) {
    ...
    if (e1.windDelta == 0 || e2.windDelta == 0) {
      ...
      if (e1.windDelta == 0 && e2.windDelta == 0) {
        ...
        if (e1.windDelta == 0 && Math.abs(
          e2.windCnt) == 1 && (clipType != ClipType.UNION || e2.windCnt2 == 0)) {
          addOutPt(e1, pt);
          if (e1Contributing) {
            e1.outIdx = Edge.UNASSIGNED;
          }
        } else if (e2.windDelta == 0 && Math.abs(
          e1.windCnt) == 1 && (clipType != ClipType.UNION || e1.windCnt2 == 0)) {
          addOutPt(e2, pt);
          if (e2Contributing) {
            e2.outIdx = Edge.UNASSIGNED;
          }
    ...
    while (e != null) {
      if (e.isIntermediate(topY)) {
        ...
        if (ePrev != null && ePrev.getCurrent().getX() == e.getBot().getX() && ePrev.getCurrent().getY() == e.getBot().getY() && op != null
          && ePrev.outIdx >= 0 && ePrev.getCurrent().getY() > ePrev.getTop().getY() && Edge.slopesEqual(
          e, ePrev) && e.windDelta != 0
          && ePrev.windDelta != 0) {
          ...
          && eNext.outIdx >= 0 && eNext.getCurrent().getY() > eNext.getTop().getY() && Edge.slopesEqual(
          e, eNext) && e.windDelta != 0
          && eNext.windDelta != 0) {
