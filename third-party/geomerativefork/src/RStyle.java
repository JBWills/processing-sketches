...
public class RStyle {
  ...
  protected void setStyle(String styleString) {
    ...
    String[] styleTokens = PApplet.splitTokens(
      styleString,
      ";"
    );
  ...
  public void setStrokeWeight(String str) {
    ...
    if (str.endsWith("px")) {
      setStrokeWeight(PApplet.parseFloat(str.substring(
        0,
        str.length() - 2
      )));
    } else {
  ...
  private int getColor(String colorString) {
    ...
    if (colorString.startsWith("#")) {
      ...
      String[] rgb = PApplet.splitTokens(
        colorString,
        "rgb( , )"
      );
      return RG.parent().color(
        PApplet.parseInt(rgb[0]),
        PApplet.parseInt(rgb[1]),
        PApplet.parseInt(rgb[2])
      );
    } else {
